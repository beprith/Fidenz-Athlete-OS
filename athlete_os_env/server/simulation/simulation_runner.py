...
class SimulationRunner:
    ...
    def run_round(self, config: SimConfig, round_num: int) -> RoundSummary:
        ...
        stats = self.engine.compute_round_stats(events)

        # Compute fatigue delta
    ...
    def run_scenario(
        ...
        metrics = {
            ...
            "coherence_score": 0.7,  # baseline coherence
        ...
        return SimulationResult(
            ...
            player_summary=f"{persona.name} at {config.team} ({config.role}): {total_goals}G {total_assists}A, avg {avg_rating:.1f}",
